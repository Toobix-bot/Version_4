...
import time  # für Snapshot Ausgabe
from src.core.orchestrator import Orchestrator
from src.core.twin import TwinCoordinator, SnapshotManager
...
def main() -> None:
    ...
    # Twin / Snapshot Optionen
    parser.add_argument("--twin-sandbox-cycle", type=int, default=0, help="Führt N Zyklen im Sandbox-Klon aus (Twin-Modus)")
    parser.add_argument("--twin-promote", action="store_true", help="Promotet geänderte Dateien aus Sandbox zurück ins Haupt-Repo (dry-run falls --dry-run)")
    parser.add_argument("--twin-list-changed", action="store_true", help="Listet geänderte Dateien der Sandbox")
    parser.add_argument("--snapshot-create", type=str, default=None, help="Erstellt Snapshot mit Label")
    parser.add_argument("--snapshot-list", action="store_true", help="Listet Snapshots")
    parser.add_argument("--snapshot-restore", type=str, default=None, help="Snapshot ID wiederherstellen")
    ...
    if args.snapshot_create:
        meta = snaps.create(args.snapshot_create)
        print(f"[SNAPSHOT] Erstellt {meta.id} -> {meta.label}")
        return
    if args.snapshot_list:
        for s in snaps.list():
            print(f" - {s.id}: {s.label} ({time.ctime(s.timestamp)})")
        return
    if args.snapshot_restore:
        ok = snaps.restore(args.snapshot_restore)
        print("[SNAPSHOT] Restore OK" if ok else "[SNAPSHOT] Restore FEHLGESCHLAGEN")
        return

    # ---- Twin Sandbox Zyklen ---- #
    if args.twin_sandbox_cycle > 0:
        produced = twin.sandbox_cycle(cycles=args.twin_sandbox_cycle, dry_run=args.dry_run)
        if not args.json:
            print(f"[TWIN] Sandbox Zyklen abgeschlossen. Vorschläge erzeugt: {produced}")
    if args.twin_list_changed:
        ...
        if changed:
            ...
            print("[TWIN] Keine Änderungen.")
        return
    if args.twin_promote:
        promoted = twin.promote(dry_run=args.dry_run)
        if promoted:
            print("[TWIN] Promoted:")
            for pth in promoted:
                print(f" - {pth}")
        else:
            print("[TWIN] Nichts zu promoten.")
        return
    _print_simple_intro()
    import json as _json
    from src.core.models import PatchProposal  # Laufzeit Import für Typ
    all_scored: list[PatchProposal] = []
