...
class EvolutionAgent(BaseAgent):
    ...
    def __init__(self, repo_root: Optional[Path] = None) -> None:
        ...
        self.last_parse_meta: Dict[str, Any] = {"source": "init"}
        try:
    ...
    def propose(self, state: WorldState) -> List[PatchProposal]:
        if not self.client or not self.client.cfg.api_key:
            try:  # pragma: no cover
                ...
                pass
            self.last_parse_meta = {"source": "fallback:no-key"}
            return self._fallback_static(state)
        prompt_lines = [
            'Erzeuge bis zu 3 kleine, sichere Verbesserungsvorschläge für dieses Projekt. ',
            'GIB AUSSCHLIESSLICH EIN VALIDES JSON-ARRAY OHNE EINLEITUNG ODER ERKLÄRUNG AUS. ',
            'Schema: [{"id":str,"title":str,"description":str,"rationale":str,"filename":str,"content":str}]. ',
            'Regeln: id kurz (p_<slug>), nur neue oder verbesserte kleine Dateien (Doku, Guide, Config-Hinweis), kein Code der Ausführungsschäden verursacht. ',
            'Kein Markdown außerhalb des JSON. Beispiel: ',
            '[{"id":"p_example","title":"Add helper doc","description":"Short helper explanation","rationale":"Clarity","filename":"HELPER.md","content":"# Helper\nKurze Beschreibung"}]'
        ]
        ...
        if self.repo_root:  # pragma: no cover
            try:
                logs_dir = self.repo_root / "logs"
                logs_dir.mkdir(exist_ok=True)
        ...
        try:
            ...
            if not isinstance(parsed_any, list):
                parsed_any = []
            data_list: List[Dict[str, Any]] = [o for o in parsed_any if isinstance(o, dict)]  # type: ignore
            ...
            if self.repo_root:  # pragma: no cover
                try:
                    (self.repo_root / "logs" / "groq_parse_error.txt").write_text(
                        f"Error: {e}\nRAW_JSON_CANDIDATE=\n{raw_json}\nFULL_RAW=\n{raw}", encoding="utf-8"
                    )
                except Exception:
                    pass
            self.last_parse_meta = {"source": "fallback:parse-error", "raw_len": len(raw), "parsed": 0}
            return self._fallback_static(state)

        # Duplicate detection
        filtered: List[PatchProposal] = []
        for p in proposals:
            try:
                lines = p.diff.splitlines()
                target_line = next((ln for ln in lines if ln.startswith("+++ b/")), "")
                filename = target_line.replace("+++ b/", "") if target_line else ""
                content_added = [ln[1:] for ln in lines if ln.startswith("+") and not ln.startswith("+++")]
                if filename and self.repo_root:
                    fpath = self.repo_root / filename
                    if fpath.exists():
                        existing_text = fpath.read_text(encoding="utf-8", errors="ignore")
                        meaningful = [l.strip() for l in content_added if l.strip()]
                        if meaningful and all(m in existing_text for m in meaningful):
                            continue  # skip duplicate
                filtered.append(p)
            except Exception:
                filtered.append(p)
        if not filtered:
            self.last_parse_meta = {"source": "fallback:duplicates", "raw_len": len(raw), "parsed": len(proposals)}
            return self._fallback_static(state)
        self.last_parse_meta = {"source": "llm", "raw_len": len(raw), "parsed": len(proposals), "kept": len(filtered)}
        return filtered
